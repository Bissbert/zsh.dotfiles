...
2. Run these commands on a machine connected to the Internet after replacing the value of
   ...
   ```
3. Copy `~/powerlevel10k` from the machineonnected to the Internet to the one without Internet
   access.
...
### Where can I ask for nd report bugs?
...
Powelevel10k defines prompt and nothing else. It sets [prompt-related options](
...
### I'ng Powerlevel9k with Oh My Zsh. How do I migrate?
...
2. *Optional but highly recommended:*
   1. Install [the recommended font](#meo-nerd-font-patched-for-powerlevel10k).
...
*Related:*
  ...
  - [Does Powerlevel10k always render exactly the same prompt as Powerlevel9k given the same config?](
      #does-powerlevel10k-always-render-exactly-the-same-prompt-as-powerlevel9k-given-the-same-config)
  - [Extra or missing spaces in prompt compared to Powerlevel9k.](
      #extra-or-missing-spaces-in-prompt-compared-to-powevel9k)
  - [Configuration wizard.](#configuration-wizard)
...
is fully initialized, standard file descriptors are restored and the content of the tempore
is printed out.
...
aough output that normally has colors may appear uncolored. You can either leave it be, suppress
...
chatty-script                 # spams to stdout even werything is fine
# ...
```
...
instant prmpt with `POWERLEVEL9K_INSTANT_PROMPT=off`. Do this if instant prompt breaks Zsh
...
*Related*: [ow do I export GPG_TTY when using instant prompt?](
  #how-do-i-export-gpg_tty-when-using-instant-prompt)
...
*Related*: [How do I initialize direnv when usininstant prompt?](
  #how-do-i-initialize-direnv-when-using-instant-prompt)
...
| `=`       | up to date wth the remote (neither ahead nor behind)                | `git rev-list --count HEAD...@{upstream}`              |
...
*Related*: [What do different symbols in Git status mean?](
  #what-do-diferent-symbols-in-git-status-mean)
...
To see Git stfor `$HOME/.git` in prompt, open `~/.p10k.zsh` and remove
...
statusfter every command. If the repository is large, or the machine is slow, this computation
...
When using *Rainbow* style, Git status is disas black on grey while it's still being
...
indication that you are working locally and that you aren't root. You can change it, however.n `~/.p10k.zsh`. Close to the top you can see the most important parameters that define which
...
You can ave `context` to a different position in `POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS` or even
...
igs created by `p10k configure` may contain parameters of this kind. To customize when different
...
function kube-toggle() {
  if (( ${+POWERLEVEL9K_KUBECONEXT_SHOW_ON_COMMAND} )); then
...
You canither [change the color palette used by your terminal](
...
When you change the trminal color palette, it usually affects only the first 16 colors, numbered
...
Open `~/.p10k.zsh`, search for "color", "foregrou" and "background" and change values of
...
Type `source ~/.p10k.zsh` toly your changes to the current Zsh session.
...
Powerlevel10k uses [gitstatus](https://ghub.com/romkatv/gitstatus) as the backend behind `vcs`
...
N Powerlevel10k is always fast, with any configuration you throw at it. If you have noticeable
...
[powerlevel9k#1170](https://github.com/Powerlevowerlevel9k/issues/1170). Powerlevel9k was
...
that have commit rights in both repositories. All bug fixes and new features coed to
...
Power10k understands. Names of all parameters in Powerlevel10k start with `POWERLEVEL9K_` for
consistency.
...
- Powerlevel10k doesn't supportWERLEVEL9K_VCS_SHOW_SUBMODULE_DIRTY=true`.
- Powerlevel10k strives to be bug-compatible with Powerlevel9k but not when it comes to egregious
  ...
  - Powerlevel9k ignores some options that are set after the theme is sourced while Powerlevel10k
    ...
    probably defined `POWERLEVEL9K_MODE` before sg the theme. This parameter gets ignored
  ...
  - Powerlevel9k doesn't respect `ZLE_RPROMPT_INDENT`. As a result, right prompt in Powerlevel10k
    ...
    [troubleshooting](#extra-space-without-background-on-the-right-sideight-prompt).
...
Powerlevel10k, please [open an issue](https://github.com/romkatv/plevel10k/issues).
...
If you want to confinept colors to the selected terminal color palette (say, *Solarized Dark*),
...
extra space for typing commands without the al drawback of reduced scrollback density. Having
...
and replace mode respectively. When a command fails, the symbol turns red. *Lean* style always `prompt_char` in it. *Rainbow* and *Classic* styles have it only in the two-line configuration
without left frame.
...
- Use * icons*. The extra icons enabled by the *many icons* option primarily serve decorative
...
[the recommended font](#meslo-nerd-patched-for-powerlevel10k) and Tango Dark color palette with
...
[The recommended font](#meslo-nerd-font-patched-for-powerlevelk) is the product of many
...
```zsh
git clone th=1 https://github.com/romkatv/nerd-fonts.git
cd nerd-fonts
./build 'Meslo/S/*'
```
...
- [`[oh-my-zsh] heme 'powerlevel10k/powerlevel10k' not found`](#oh-my-zsh-theme-powerlevel10kpowerlevel10k-not-found)
- [Question mark in prompt](#question-mark-in-prompt)
- [Icons, glyphs or powerline symbols don't render](#icons-glyphs-or-powerline-symbols-dont-render)
...
- [Prompt wrapping aroud in a weird way](#prompt-wrapping-around-in-a-weird-way)
...
- [Extra or missing spaces in promt compared to Powerlevel9k](#extra-or-missing-spaces-in-prompt-compared-to-powerlevel9k)
...
nnot make Powerlevel10k work with my plugin manager](#cannot-make-powerlevel10k-work-with-my-plugin-manager)
- [Directory is difficult to see in prompt when using Rainbow style](#directory-is-difficult-to-see-in-prompt-when-using-rainbow-style)
- [Incorrect foreground color in VSCode Terminal.](#incorrect-foreground-color-in-vscode-terminal)
...
- [Arch Linux logoas a dot in the bottom right corner](#arch-linux-logo-has-a-dot-in-the-bottom-right-corner)
...
You can also get a weird-looking question mark in your prompt if your terminal's font is missiome glyphs. See [icons, glyphs or powerline symbols don't render](
...
There are three imperfectns on the screenshot. From left to right:
...
screen is made of monospace characters. Ate powerline prompt segment is made of text on white
...
- Shift all text one pixel up/down/left/right i terminal has an option to do so.
...
doesn't support UTF-8. You need to fix it. If you are running Zsh oveSSH, see
...
#### If the prompt line is longthan the frame
...
### Right prompt is in the wronlace
...
configuration wizard. Once you can see the errs, fix `~/.zshrc` to get rid of them.
...
`MULTIBYTE` shell option is disabled. *Fix*: Enable the `MUTIBYTE` option, or rather don't disable
...
[the recommended font](#meslo-nerd-font-patch-powerlevel10k). Verify by running
...
the same prompt spacing as in Pol9k.
...
>
> If set, used to give the indentation between the right hand side of the prompt in the line
...
you set `ZLE_RPROMPT_INDENT=0`, your prompt o to the edge of the terminal. This is how it
...
icons as in Powerlevel9kSpacing around icons in Powerlevel9k is inconsistent.
...
1. Disable the current theme in your fra/ plugin manager.
...
- **prezto:** Open `~/.zpreztorc` and put `zstyle :prezto:module:prompt theme off` in it. Remove
  any other command that sets `` such as `zstyle :prezto:module:prompt theme powerlevel9k`.
...
### Directory is difficult to  prompt when using Rainbow style
...
- [Change terminal color palette](#change-the-color-palette-used-by-your-terminal). Try Tango 
...
*Related*: [Incorrect foreground color in VSCode Terminal.](#incorrect-foround-color-in-vscode-terminal)
...
Terminal content before hrinking:
...
print -l {1..3}
setopt prompt_subst
PROMPT=$'${$((pause()))+}le{(pl.$((COLUMNS-12))..-.)}<right\n> '
```
...
terminal content that follows and printseexpanded prompt there. However, after resizing prompt is
...
higher than intended, potentiasing useful content in the process.
...
- Simple one-line left prompt. No `prompt_subst`, no right prompt. Here `VD` is bound to increaspon terminal shrinking due to the command line wrapping around.
...
printing prompt and `rc` (restore cursor) to move cursor back to the original position when pro
...
position). In other words, the patch fixes the resizing issue on some terminals while keeping thior unchanged on others.
...
- Recompute `VD` based on new terminal dimensions before attempting to refresh prompt. This doesn't
  work because Zsh doesn't know whetheinal reflows text or truncates it. If Zsh could somehow
...
There are a few migation options for this issue.
...
- Apply [the patch](#zsh-patch) and [rebuild Zsh from source](
  ...
  On sucminals the patch will have no visible effect.
...
- Avoid long lines between the start of prompt and cursor.
  ...
  3. Disable right frame with `POWERLEVEL9K_MULTILINE_FIRST_PROMPT_SUFFIX=''`,
     `POWERLEVEL9K_MULTILINE_NEWLINE_PROMPT_SUF=''` and
...
"non-mpace" refers to any font with glyphs wider than a single column, or wider than two columns
...
2. Use a monospace fontanually add an extra space after the icon that gets cut off. For example, if the content of
...
```zsh
typeset -g POWERLEVEL9K_LIN_ARCH_ICON='Arch'  # plain "Arch" in place of a logo
```
...
2. Display the icon in regular (non-bold) typeface. To do this, open `~/.p10k.zsh`, search for
   `POWERLEVEL9K_OS_ICON_CONTENT_EXPANSION` and rve `%B` from its value.
```zsh
...
the real state of a git repository (reflected by `git status`what gets shown in the
